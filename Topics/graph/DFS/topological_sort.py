...
def kahn_topological_sort_wiki(graph):
    in_degree_dict = get_indegree_dict(graph)
    no_indegree = deque([v for v in in_degree_dict if in_degree_dict[v] == 0])
    top_sort = []

    while no_indegree:
        node = no_indegree.popleft()
        top_sort.append(node)
        for nbr in graph[node]:
            in_degree_dict[nbr] -= 1
            if in_degree_dict[nbr] == 0:
                no_indegree.append(nbr)
        graph.pop(node)
